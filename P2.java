/* Akshata Dhuraji, c3309266
 Operating System Assignment2 - Problem 2 : Covid-safe restaurant using Semaphore
 ...
 First we create the semaphores.As there are no customers and the restaurent is empty so we call the constructor with parameter 0 thus creating semaphores with zero initial permits.
...
public class P2 extends Thread {  
    ...
    public static final int CHAIRS = 5;     // we denote that the number of chairs in this restaurent is 5.
    public static final int cleaningTime=5; //cleaning time taken by the restaurent
    ...
    int custBatch=0;            //Batch of customers entering restaurent
...
class Customer extends Thread {
  ...
  public Customer(int at, int et, String cs) {
    ...
    leaves=0;               				//Time when the customer leaves the restaurent,computed based on the assignment rules
  }
    ...
    while (notEating) {                 	// as long as the customer is not eating 
      try {
          ...
          if (numberOfFreeSeats > 0) {  	//if there are any free seats
              ...
              customers.release();      	//notify the restaurent that there is a customer
              this.eating();            	//eating...
              accessSeats.release();    	// don't need to lock the chair for customer anymore as the customer has left  
              try {
                  seat.acquire();       	// now it's this customers turn but we have to wait if the restaurent is busy
  ...
  /* this method calculates the customer seating time and leaving time from the restaurent */ 
  public void getLeavingTime(){
      seats=arrivalTime;
      leaves= eatingTime + seats;
      if(nextBatch){								//boolean value to check if the next batch has arrived
           lastLeavingTime=temp;					//transfer the time of customer who left last from the restaurent into a temp variable 
           nextBatch=false;							//change the value to false untill the following batch arrives
      }
  ...
  /* this method will simulate customer is eating in the restaurent and prints the customer details */
  public void eating(){
...
class Seats extends Thread {
  ...
  public void run() {
    while(true) {                       // runs in an infinite loop
      ...
      if(numberOfFreeSeats==0){
            seat.release();             // the restaurent is ready to serve
  ...
  public void serveNextBatch(){
    //System.out.println("The restaurent is ready to serve next batch of customer");//Debug code
 ...
 /* main method */
  public static void main(String args[]) {
    ...
    String myStr;                               //String to hold each line of the file untill the last line is read
    int i=0;                                    //counter to check the number of lines in the file
    String inFile=" " ;
    P2 p2 = new P2();                            //Creates a new restaurent
    ...
    BufferedReader br = new BufferedReader(file);//Read the file 
     ...
     while((myStr = br.readLine()) != null){    //Read the file line by line
        ...
        i++;                                    //Moves to the next customer details,untill the last customer details are read
     }
    } catch (IOException e){ 
