...
class Restaurent {
    ...
    Restaurent(int freeChair) {
        ...
        cleaningTime=0;             //time to clean the restuarent after the last customer from the batch has left
        custBatch=0;                //number of customer batch
    }

    // as customers enter the restaurent, first 5 seat themselves rest will queue up untill cleaning is done 
    ...
    void enterShop() throws InterruptedException {
        ...
        // when the customer arrives he waits for restuarent to allow him in
        // if the restaurent is serving the first batch customer waits in line - nothing happens here
        synchronized (this) {
            notifyAll();
        }
    }
    ...
    synchronized void work() throws InterruptedException {
        ...
        // if there are no customers the restaurent waits for the new customers.
        
        while (true) {
            while (numOfFreeChair.size() == 0) {
                //System.out.println("new customer is waiting outside restaurent for..."); debug code
                wait();
            }
            serveCustomers();
        }
    ...
    private void serveCustomers() throws InterruptedException {
        while (!numOfFreeChair.isEmpty()) {
            // Restaurent allows customer in and serves him
    ...
    synchronized void getServed(Customer customer) throws InterruptedException {
       // System.out.println("The restaurent is serving " + customer.custnum + "..."); //--Debug code
        iD++;               //customer counter
        if (iD>custAllowed){//if the number of customer > number of customer allowed in the restaurent then restaurent rules are trigger
           for(int i=0;i<5;i++)
                numOfFreeChair.poll();  //restaurent is being cleaned up - removes the record of previous customer batch
           cleaningTime=5;              //cleaning time after the previous batch of customers have left
           nextBatch=true;              //variable to confirm nextbatch is allowed in the restaurent post clean-up
           custAllowed=custAllowed+5;   //variable to check the number of customer inside the restaurent is not more than 5 at any time
        }
        eating();
       // System.out.println(customer.custnum + "is done");//--Debug code
    }

    // Customer is enjoying the restaurent food
